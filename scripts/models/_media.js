...
export class Media extends Api {
  ...
  async getAllMedia(photographerId) {
    try {
      ...
      const MediaImage = getMediaApi.map(media => new MediasFactory(media, 'MediaImage'))
      const MediaVideo = getMediaApi.map(media => new MediasFactory(media, 'MediaVideo'))

      const FullMedias = MediaImage.concat(MediaVideo)
